...
@Component({
  selector: 'app-aluno-curso-turma',
  templateUrl: './aluno-curso-turma.component.html',
  styleUrls: ['./aluno-curso-turma.component.scss']
})
export class AlunoCursoTurmaComponent implements OnInit, OnDestroy {
  @Input() id;
  ...
  openSelectTurma(changeTurma: boolean = false): void {
    const dialogRef = this.dialog.open(AlunoCursoTurmaSelectComponent, {
      width: '90vw',
      autoFocus: false,
      data: { }
    });
  ...
  goToAulasOnline(matriculaId: number, materiaId: string): void {
    window.localStorage.setItem('materiaIdLocalStorage', materiaId);
  ...
  getAulas(): Promise<any> {
    ...
    return new Promise((res, rej) => {
      this.aulaOnlineService.getMateriasPorCurso(this.cursoId).subscribe(val => {
        if (val?.status === 'error') {
          this.error = true;
          rej();
          return;
        ...
        this.materias = val;
        this.nomeAulaOnline = val?.nomeMateria;
        res(val);
        return;
      });
    });
