...
namespace EscolaPro.API.Controllers
{
    ...
    public class AtendimentoOutboundController : ControllerBase
    {
        ...
        //[Authorize]
        [HttpGet]
        public async Task<IActionResult> ListaOutbound(int idUnidade)
        {
            try
            {
                var retorno = await _atendimentoOutbound.Outbound(idUnidade);
                return Ok(retorno);
            }
            catch (Exception ex)
            {
                RegistraLog.Log(ex.Message, TipoResquisicao.Error, 
                    $"{nameof(ListaOutbound)}", $"{nameof(AtendimentoOutboundController)}");
