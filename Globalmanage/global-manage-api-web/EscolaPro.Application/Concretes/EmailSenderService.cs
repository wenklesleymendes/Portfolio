...
namespace EscolaPro.Service.Concretes
{
    public class EmailSenderService : IEmailSenderService
    {
        ...
        public EmailSenderService(IOptions<EmailSettings> emailSettings, IOptions<AppSettings> appSettings, IReguaContatoHistoricoService reguaContatoHistoricoService, IAlunoRepository alunoRepository)
        {
        ...
        public async Task SendEmailAsync(string[] email, string subject, string message, List<Attachment> attachments, bool nacionalTec = false, AlternateView alternateView = null, int alunoId = 6)
        {
            try
            {
                 var aluno = await _alunoRepository.BuscarPorId(alunoId);
                if (!aluno.Contato.ReceberEmail && alunoId != 6)
                    return ;
