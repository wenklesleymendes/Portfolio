    ...
    public class ProvaAlunoService : IProvaAlunoService
    {
        ...
        public async Task<DtoProvaAluno> Inserir(DtoProvaAlunoRequest dtoProvaAluno)
        {
            ...
            if (matricula.UnidadeId.HasValue)
            {
                ...
                if (dtoProvaAluno.TipoProva == TipoProvaEnum.Presencial && matriculaMail.Aluno.Contato.ReceberEmail)
                {
                ...
                {
                    if (matriculaMail.Aluno.Contato.ReceberEmail)
                    {
                ...
                if (matriculaMail.Aluno.Contato.ReceberWhatsApp)
        ...
        public async Task<DtoProvaAlunoInformacoes> InformacoesCadastro(int matriculaId)
        {
            ...
            if (matricula.PlanoPagamentoAluno.PlanoPagamentoId.HasValue)
            {
                ...
                var pagamentoIdOlds = pagamentos.Where(x => x.PagamentoIdOld.HasValue).Select(x=> x.PagamentoIdOld).ToList();

                if (pagamentos.Any(x => !pagamentoIdOlds.Contains(x.Id) &&
        ...
        private async void EnviarWhatsAppIncrição(DtoProvaAlunoRequest dtoProvaAluno, MatriculaAluno matricula)
        {
            ...
            string retorno = await _whatsAppService.SendMessage(matricula.Unidade.Contato.Instance, matricula.Unidade.Contato.Token, matricula.Aluno.Contato.Celular, mensagemWhatsApp, matricula.Aluno.Id); // teste salvar aluno na tabela historico regua contato 
        }
        ...
        private async void EnviarWhatsAppCredencialProvaOnline(MatriculaAluno matricula, ProvaAluno provaAluno)
        {
            ...
            string retorno = await _whatsAppService.SendMessage(matricula.Unidade.Contato.Instance, matricula.Unidade.Contato.Token, matricula.Aluno.Contato.Celular, stringBuilder.ToString(), matricula.Aluno.Id); // teste salvar aluno na tabela historico regua contato 
        }
    }
}
