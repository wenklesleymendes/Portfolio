...
namespace TcpIp
{
    public class TcpIpServidor
    {
        ...
        private void EncerraClientes()
        {
            ...
            {
                AuditoriaLog.EscrevaErro(nameof(TcpIpServidor), ex);
                LogAuditoria.Escreva(nameof(TcpIpServidor), "Conexão com o Cliente do Monitor foi encerrada");
            }
        }
        ...
        private static void RemoveClientesInativos()
        {
            ...
            {
                AuditoriaLog.EscrevaErro(nameof(TcpIpServidor), ex);
                LogAuditoria.Escreva(nameof(TcpIpServidor), "Cliente do Monitor encerrou comunicação");
            }
        }

        public void IniciarServidorTcpIp(string ip, string porta, string tipoServidor, bool EhParaMudarCaminho = false)
        {
            LogAuditoria.Escreva(nameof(TcpIpServidor), $"Inicando {nameof(IniciarServidorTcpIp)}({ip},{porta},{tipoServidor},{EhParaMudarCaminho})");
            ...
            for (int i = 0; i < tcpCliente.Length; i++)
            {
                if (tcpCliente[i].Client.Connected)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        nomeIdentificadorDaConexao.Remove(tcpCliente[i]);
                        AuditoriaLog.EscrevaErro(nameof(TcpIpServidor),ex);
                        LogAuditoria.Escreva(nameof(TcpIpServidor), "Falha ao enviar dados ao Cliente do Monitor");
                    }
                }
            }
        }
        ...
        private class IniciaComunicacaoTcpIp
        {
            ...
            {
                LogAuditoria.Escreva(nameof(TcpIpServidor), "Iniciando comunicação com novo Cliente do Monitor");
            ...
            public void MonitoraMensagens(TcpClient tcpCliente, string ip, int porta)
            {
                ...
                {
                    LogAuditoria.Escreva(nameof(TcpIpServidor), "Tentativa de conexão sem sucesso");
                }
                catch (Exception ex)
                {
                    executarCliente = false;
                    AuditoriaLog.EscrevaErro(nameof(TcpIpServidor), ex);
                    LogAuditoria.Escreva(nameof(TcpIpServidor), "Cliente do Monitor desconectou");
                }
                finally
                {
