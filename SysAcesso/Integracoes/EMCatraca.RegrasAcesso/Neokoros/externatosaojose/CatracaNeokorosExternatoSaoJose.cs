...
{
	internal class CatracaNeokorosExternatoSaoJose : CatracaNeokorosAbstract
	{
		...
		public CatracaNeokorosExternatoSaoJose(Dispositivo Catraca) : base(Catraca)
		{
		...
		{
			LogAuditoria.Escreva($"Iniciando Obtenha TurmaMontada -> código:{codigo} ", nameof(CatracaNeokorosExternatoSaoJose));
			...
			if (PessoaAluno.Equals(TipoPessoa.Aluno))
			{
				...
				return _repositorioAluno.ConsulteTurmaMontadaPorAluno(Convert.ToInt32(matricula));
			}

			throw new AcessoNegadoException($"Ets:{PessoaAluno}");
		}
